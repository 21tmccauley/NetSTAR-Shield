...
// This is the code for the toggle switch
...
// Gives the value for the RiskMeter Circle, and the circle
...
// Fills Risk meter bar basked on value given for the category
...
function Summary({ state }) {
  ...
  return (
    <div>
        ...
        <div>
          ...
          <div className="small" style={{ marginBottom: 8 }}>
            {state.verdict === "Caution"
              ...
              : "Safe to browse, routine caution advised."}
          </div>
      ...
      <div><SecurityAnalysis metrics={state.metrics}/></div>
    </div>
  );
}

function SecurityAnalysis({metrics}) {
  return (
    <div>
      <div className="section-title">Security Analysis</div>
      <ProgressRow
        label="Domain Reputation"
        value={metrics.domainReputation}
      />
      <ProgressRow
        label="Domain Signals"
        value={metrics.domainSignals} />
      <ProgressRow
        label="Certificate Trust"
        value={metrics.certificateTrust}
      />
      <ProgressRow
        label="Connection Security"
        value={metrics.connectionSecurity}
      />
      <ProgressRow
        label="Credential Safety"
        value={metrics.credentialSafety}
      />
      <ProgressRow
        label="Page Content"
        value={metrics.pageContent} />
    </div>
  );
}
...
function Details() {
  return (
    <div className="panel">
      ...
      <ul className="list">
        <li>Signals and evidence items here…</li>
      </ul>
    </div>
  );
}
  ...
  return (
    <div className="panel">
      ...
      <div className="toggle">
        <span
          style={{ fontWeight: 600, color: enabled ? "#16a34a" : "#475569" }}
        >
          {enabled ? "On" : "Off"}
        </span>
        <Switch
...
export default function App() {
  ...
  return (
    <div>
      <div className="header">
        ...
        <div className="toggle">
          <span
            style={{
              fontWeight: 600,
              color: settings.enabled ? "#16a34a" : "#475569",
            }}
          >
      ...
      {active === "Settings" && (
        <Settings settings={settings} update={updateSettings} />
      )}
