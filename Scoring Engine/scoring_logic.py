...
def score_cert_health(cert_data: dict, scan_date: datetime, scores: dict): #TODO: Change data to cert_data and fix any waterfall affect from it
    ...
    try:
        ...
        connection_data = cert_data.get("connection", {})
        verification_data = cert_data.get("verification", {})
...
def score_dns_rec_health(dns_data: dict, scores: dict): 
...
def score_conn_sec(hval_data: dict, cert_data: dict, scores: dict): 
    ...
    if final_status == 403:
        ...
        if app_config.VERBOSE:
            print("HVAL Score: CRITICAL - No response from server (connection failed). (CONN_SEC)")
...
# --- Mail Scan ---
    ...
    print(f"Target TLD: {target_tld}")
    if target_tld in app_config.MAL_TLDS_SLIM:
        scores['Domain_Reputation'] -= 10
        if app_config.VERBOSE:
            print(f"RDAP Score: Minor Deduction - TLD '{target_tld}' is associated with malicious websites. -10 (DOM_REP)")
    else:
        pass
...
def score_cred_safety(cert_data:dict, hval_data:dict, scores:dict): 
    ...
    if not cert_list or not isinstance(cert_list, list) or len(cert_list) == 0:
        if app_config.VERBOSE:
            print("Credential Safety: CRITICAL - No certificates found in response. (CRED_SAFETY)")
    ...
    if key_algorithm not in ['ECDSA', 'RSA']:
        scores['Credential_Safety'] -= 20
        if app_config.VERBOSE:
            print("Cred Safety Score: Significant Deduction - Certificate key algorithm missing. (CRED_SAFETY)")   
    ...
    if (key_algorithm == "ECDSA"):
        ...
        if (curve_name not in ['P-256', 'P-384', 'P-521']):
            ...
            if app_config.VERBOSE:
                print(f"Cred Safety Score: Significant Deduction - Outdated curve: {curve_name}. (CRED_SAFETY)")
        if (sig_algorithm not in ['ECDSA-SHA256', 'ECDSA-SHA384', 'ECDSA-SHA512']):
            scores['Credential_Safety'] -= 5
            if app_config.VERBOSE:
                print(f"Cred Safety Score: Minor Deduction - Weak signature algorithm: {sig_algorithm}. (CRED_SAFETY)")
    ...
    if (key_algorithm == "RSA"):
        ...
        if (key_size not in [2048, 3072, 4096]):
            scores['Credential_Safety'] -= 15
            if app_config.VERBOSE:
                print(f"Cred Safety Score: Significant Deduction - Outdated key size: {key_size}. (CRED_SAFETY)")
        if (sig_algorithm not in ['SHA256-RSA', 'SHA384-RSA', 'SHA512-RSA']):
            scores['Credential_Safety'] -= 5
            if app_config.VERBOSE:
                print(f"Cred Safety Score: Minor Deduction - Weak signature algorithm: {sig_algorithm}. (CRED_SAFETY)")

def score_ip_rep(firewall_data:dict, scores:dict): 
    """IP Reputation scoring function. Checks if IP is on firewall list by FireHOL and scores accordingly."""
    blocked = firewall_data.get("Block", False)
    if blocked:
        scores['IP_Reputation'] -= 99
        if app_config.VERBOSE:
            print("IP Reputation Score: CRITICAL - IP is listed on a firewall blocklist. (IP_REP)")
    else:
        if app_config.VERBOSE:
            print("IP Reputation Score: No deduction - IP is not listed on firewall blocklist. (IP_REP)")

def score_whois_pattern(rdap_data:dict, scan_date: datetime, scores:dict): 
    """WHOIS Pattern scoring function. Checks for domain age, registrar reputation, and domain status patterns."""
    domain_data = rdap_data[0].get('domain', {})
    host = rdap_data[0].get("host","")  
    nameservers = rdap_data[0].get("nameserver", [])
    events = domain_data.get('events', [])
    status = domain_data.get('status', []) 
    #place to check for "client delete prohibited",
    ...
    if "add period" in status:
        ...
        if app_config.VERBOSE:
            print("WHOIS Score: CRITICAL - Domain is newly registered (add period). (WHOIS_PATTERN)")

    # Check for client and server locks
    for lock in app_config.CLIENT_LOCKS:
        if lock not in status:
            scores['WHOIS_Pattern'] -= 5
            if app_config.VERBOSE:
                print(f"WHOIS Score: Minor Deduction - {lock} is not set. (WHOIS_PATTERN)")

    for lock in app_config.SERVER_LOCKS:
        if lock not in status:
            scores['WHOIS_Pattern'] -= 5
            if app_config.VERBOSE:
                print(f"WHOIS Score: Minor Deduction - {lock} is not set. (WHOIS_PATTERN)")
    ...
    if registration_date:
        try:
            ...
            if days_old < 30:
                ...
                if app_config.VERBOSE:
                    print(f"WHOIS Score: WARNING - Domain is very new ({max(0, days_old)} days old). (WHO_IS)")
                scores['WHOIS_Pattern'] -= 30
            else:
                if app_config.VERBOSE:
                    print(f"WHOIS Score: INFO - Domain age is {days_old} days. (WHO_IS)")

        except (ValueError, TypeError) as e:
            # Handles malformed RDAP date strings
            if app_config.VERBOSE:
                print(f"WHOIS Score: ERROR - Registration date malformed: {e}. (WHO_IS)")
            scores['WHOIS_Pattern'] -= 5 
    else:
        # If no registration event was found in the RDAP data
        if app_config.VERBOSE:
            print("WHOIS Score: WARNING - No registration date found. (WHO_IS)")
    ...
    if app_config.VERBOSE:
        print("WHO_IS Score: Registrar name is ", registrar_name)
    ...
    if found_matches:
        scores['WHOIS_Pattern'] -= 10
        if app_config.VERBOSE:
            print("WHO_IS Score: Suspicious Registrar found: ", registrar_name)
    else:
        if app_config.VERBOSE:
            print("WHO_IS Score: Trustworthy Registrar found: ", registrar_name)
...
def calculate_security_score(all_scans: dict, scan_date: datetime) -> dict:
    ...
    try:
        ...
        print(f"Error in cert_health scan: {e}")

    try:
        score_dns_rec_health(all_scans['dns_scan'], scores)
    except Exception as e:
        print(f"Error in dns_rec_health scan: {e}")

    try:
        score_conn_sec(all_scans['hval_scan'], all_scans['cert_scan'], scores)
    except Exception as e:
        print(f"Error in conn_sec scan: {e}")

    try:
        score_dom_rep(all_scans['mail_scan'], all_scans['rdap_scan'], scores)
    except Exception as e:
        print(f"Error in dom_rep scan: {e}")

    try:
        score_cred_safety(all_scans['cert_scan'], all_scans['hval_scan'], scores)
    except Exception as e:
        print(f"Error in cred_safety scan: {e}")

    try:
        score_whois_pattern(all_scans['rdap_scan'], scan_date, scores)
    except Exception as e:
        print(f"Error in whois_pattern scan: {e}")

    try:
        score_ip_rep(all_scans['firewall_scan'], scores)
    except Exception as e:
        print(f"Error in ip_rep scan: {e}")
